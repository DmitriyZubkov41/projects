...
def open_page(browser):
    # Зададим время на открытие страницы, если по истечении этого времени страница не загрузилась, нажимаем Esc
    ...
    # Конец открытия страницы 
    end_download = time.perf_counter()
    print("Время потраченное на загрузку страницы = ", int(end_download - start_download), "секунд")
...
def get_city_in_menu(browser, list_cityes):
    ...
    for city in cityes:
        if city.text not in list_cityes:
            ...
            city.click()
            time.sleep(3)
            # Нажимаем клавишу Esc (иногда страница не успевает загрузиться)
            browser.find_element(By.TAG_NAME, 'body').send_keys(Keys.CONTROL +'Escape')
            break
...
def scroll_page(count_city, browser):
    ...
    # Будем прокручивать страницу (нажимать Показать еще) пока количество отелей не будет > count_city
    ...
    while True:
        ...
        if count > count_city:
            ...
            return (list_description_hotels, country)
        else:
            try:
...
def write_in_table(tuple_hotels_country, list_cityes, wb):
    # ТАБЛИЦА hotel.xlsx
    ...
    for i in range(len(tuple_hotels_country[0])):
        print("i=", i+1)
        row = i + 2
        ...
        # Имя отеля
        name_hotel = tuple_hotels_country[0][i].find_element(By.CSS_SELECTOR, 'h3[class="HotelName_title__1D_6z"]')
        ...
        # Страна тура
        sheet[f'C{row}'] = tuple_hotels_country[1]

        # Получаем url изображения img alt="" class="Slide_embla__slide__img__j26Zc"
        url = tuple_hotels_country[0][i].find_element(By.CSS_SELECTOR, 'img').get_attribute("src")
        print("url=", url)
        ...
        min_price = tuple_hotels_country[0][i].find_element(By.CSS_SELECTOR, 'span[class="sc-hmdomO dffjpr"]').text
        sheet[f'E{row}'] = min_price

        #Дата тура с span class="Duration_date__MNy_I">27 апр по span class="Duration_date__MNy_I">3 мая
        data = tuple_hotels_country[0][i].find_elements(By.CSS_SELECTOR, 'span[class="Duration_date__MNy_I"]')
        sheet[f'F{row}'] = f'с {data[0].text} по {data[1].text}'
        

def main():
    start_time = time.perf_counter()  # время начала работы программы
    ...
    while True:
        ...
        tuple_hotels_country = scroll_page(36, browser)
        write_in_table(tuple_hotels_country, list_cityes, wb)

    # сохраняем все листы
    wb.save("hotel.xlsx")
    end_time = time.perf_counter()  # время конца записи таблицы
    print("Время, потраченное на запись в hotel.xlsx = ", int(end_time - start_time), "секунд")
